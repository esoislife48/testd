...
--A reference to the script acting as a lookup table for the dialogue
local DIALOGUE_LOOKUP = script:GetCustomProperty("DialogueLookup"):WaitForObject().context
...
function PlayDialogue(lookupKey)
	...
	local lines = DIALOGUE_LOOKUP.lookup[lookupKey]
...
function DoFlagCheck(flagCheck)
	...
	if flagCheck ~= nil and flagCheck.comparison ~= nil and flagCheck.lValueKey ~= nil then

		--Grab the lValue from the local player
		local lValue = Game.GetLocalPlayer():GetResource(flagCheck.lValueKey)

		--Perform the comparison
		passed = flagCheck.comparison(lValue, flagCheck.rValue)
	end
...
function TraversePath(path)
	...
	for i = 1, #lines do
		...
		elseif not failedCheck then
			...
			SetSpeaker(lines[i].speaker, lines[i].name, lines[i].image)
...
function SetSpeaker(speaker, name, portraitImage)
